(function () {
    ...
    // -----------------------------------------------------------------
    // -----------------------------------------------------------------
    // 5. FLUXO SEQUENCIAL DE RESPOSTAS (COM ROLAGEM DE COMENTÁRIOS)
    // -----------------------------------------------------------------
    async function processarRespostasSequenciais() {
        let contador = 1;
        let tentativasSemNovosComentarios = 0;
        const MAX_TENTATIVAS_SCROLL = 5; // Quantas vezes tentar rolar se não achar novos comentários
        ...
        while (roboAtivo) {
            ...
            // Se não encontrou nenhum comentário novo visível no momento:
            if (!proximoSpan || !usuarioAtual) {
                tentativasSemNovosComentarios++;

                if (tentativasSemNovosComentarios <= MAX_TENTATIVAS_SCROLL) {
                    console.log(`📜 Robô: Fim dos comentários visíveis na tela. Rolando painel para carregar mais... (Tentativa ${tentativasSemNovosComentarios}/${MAX_TENTATIVAS_SCROLL})`);

                    // Tenta encontrar o container rolável de comentários (geralmente uma div com overflow vertical)
                    // No Instagram/Reels, o container de comentários fica próximo ao modal
                    const modalComentarios = document.querySelector('div[role="dialog"]');
                    let containerScroll = null;

                    if (modalComentarios) {
                        // Busca o elemento interno que possui scroll
                        const elementos = modalComentarios.querySelectorAll('div');
                        for (const el of elementos) {
                            if (el.scrollHeight > el.clientHeight && getComputedStyle(el).overflowY.match(/scroll|auto/)) {
                                containerScroll = el;
                                break;
                            }
                        }
                    ...
                    if (containerScroll) {
                        containerScroll.scrollBy({ top: 400, behavior: 'smooth' });
                    } else {
                        // Fallback: tenta rolar a janela ou o último botão encontrado
                        window.scrollBy({ top: 400, behavior: 'smooth' });
                    }

                    await sleepRandom(2000, 3000); // Aguarda novos comentários carregarem no DOM
                    continue; // Volta ao início do while para verificar os novos comentários
                } else {
                    console.log("🤖 Robô: Todos os comentários elegíveis deste vídeo já foram processados.");
                    break;
                }
            }

            // Resetamos o contador de tentativas assim que encontramos um comentário válido
            tentativasSemNovosComentarios = 0;
